...
OUT_DIR="${OUT_DIR:-outputs/moe_ti2v_refiner_fsdp_cp8_$(date +%Y%m%d_%H%M%S)}"
...
"$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node "$NPROC" \
  ...
  --output "$OUT_DIR/ti2v_base.mp4" \
  --refiner_output "$OUT_DIR/ti2v_refined.mp4" \
  --height "$HEIGHT" \
  --width "$WIDTH" \
  --steps "$STEPS" \
...
echo "Saved: $OUT_DIR/ti2v_base.mp4"
echo "Saved: $OUT_DIR/ti2v_refined.mp4"
